...
diamonds_new <- as.data.frame(model.matrix(~.-1, diamonds))

normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

# Normalize data except for price
diamonds_norm <- as.data.frame(lapply(diamonds_new[,-22], normalize))
diamonds_norm$price <- diamonds_new$price

# Set diamonds data to diamonds_norm
diamonds <- diamonds_norm

str(diamonds)
summary(diamonds)
```

Before moving to the analysis of the data set, we cleaned the data to prepare it for regression. Cut, color, and clarity were ordinal factors. We decided to change cut, color, and clarity to be factors which are not ordered and then convert them to dummy variables with a value of 0 if a diamond does not have that characteristic or a value of 1 if a diamond does have that characteristic. All variables except for price were normalized.
...
# RMSE = 1505.3413035, Rsquared = 0.8589778, and MAE = 949.7464466
# and only positive prices predicted:
...
```{r, cache = TRUE}
# tune_grid_rf <- data.frame("mtry" = c(1:24)) # Test out number of predictor variables chosen
...
#   random_forest_model <- randomForest(price ~ ., data = train_diamonds, ntree = 100)
#   prediction_values_random_forest <- predict(random_forest_model, test_diamonds)
#   evaluation_values <- postResample(prediction_values_random_forest, test_diamonds$price)
#   return(values, prediction_values_random_forest)
# }
```
